...
SELECT	top 100
		...
		XE.value('(/event/@name)[1]', 'varchar(250)') AS TraceName,

		XE.value('(/event/action[@name="event_sequence"]/value)[1]', 'int') AS event_sequence,
	    XE.value('(/event/action[@name="attach_activity_id"]/value)[1]', 'varchar(250)') AS attach_activity_id,
